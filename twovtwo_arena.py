...
class Arena:
    def __init__(self):
        ...
        self.p1_2_champion = None
        self.p2_champion = None
        self.p2_2_champion = None
    ...
    def add(self, side, is_main, position, name, cost, level, evo=False, zero_delay=False):
        e = cost
        if (side and ((is_main and e <= self.p1_elixir + 0.2) or ((not is_main) and e <= self.p1_2_elixir))) or (not side and ((is_main and e <= self.p2_elixir + 0.2) or ((not is_main) and e <= self.p2_2_elixir))):  # some leneincy
            ...
            if side:
                if is_main:
                    ...
                    if not e <= self.p1_2_elixir:
                        delay = (e - self.p1_2_elixir)/e_rate
            else:
                if is_main:
                    if not e <= self.p2_elixir:
                        delay = (e - self.p2_elixir)/e_rate
                else:
                    if not e <= self.p2_2_elixir:
                        delay = (e - self.p2_2_elixir)/e_rate
    ...
    def tick(self):
        ...
        self.p1_2_elixir = min(10, self.p1_2_elixir + e_rate * TICK_TIME)
        self.p2_2_elixir = min(10, self.p2_2_elixir + e_rate * TICK_TIME)
