...
class PrincessTowerAttackEntity(AttackEntity):
    ...
    def detect_hits(self, arena):
        if (distance(self.target.position, self.position) < PRINCESS_PROJECTILE_CONTACT_RANGE):
    ...
    def tick(self, arena):
        ...
        if len(hits) > 0:
            ...
            if magnitude > 0:
                direction.scale(1 / magnitude)  # Convert to unit vector
...
class KingTowerAttackEntity(AttackEntity):
    ...
    def detect_hits(self, arena):
        if (distance(self.target.position, self.position) < KING_PROJECTILE_CONTACT_RANGE):
            return [self.target] # has hit
        else:
            return [] #hasnt hit yet
...
class KingTower(Tower):
    ...
    def cleanup(self, arena):
        ...
        if not self.activated:
            ...
            if alive_towers < MAX_TOWERS: #if any towers gone
                activated = True #activate
