...
class Options:
    def __init__(self, n1, n2, n3, offset=False):
        extra = OFFSET if offset else 0
        self.op1 = SelectionBox(WIDTH/2 - 160 + extra, HEIGHT/2, 120, 120, n1)
        self.op2 = SelectionBox(WIDTH/2 + extra, HEIGHT/2, 120, 120, n2)
        self.op3 = SelectionBox(WIDTH/2 + 160 + extra, HEIGHT/2, 120, 120, n3)

    def draw(self, screen): 
        self.op1.draw(screen)
        self.op2.draw(screen)
        self.op3.draw(screen)
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            elif self.op3.is_in(*event.pos):
                return self.op3.value
        return None
...
def run_loop(screen, evo_enabled =  False, side = True):
    ...
    used.append(n2)
    n3 = random_with_param(*option_types[i], used)
    used.append(n3)

    option = Options(n1, n2, n3)

    n1 = random_with_param(*option_types[j], used)
    used.append(n1)
    n2 = random_with_param(*option_types[j], used)
    used.append(n2)
    n3 = random_with_param(*option_types[j], used)
    used.append(n3)

    option2 = Options(n1, n2, n3, True)
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if not one_done:
                choose = option.handle_event(event)
                if choose is not None: #if chose
                    ...
                    if i < 8:
                        ...
                        one_done = True
            if not two_done:    
                choose = option2.handle_event(event)
                if choose is not None: #if chose
                    all2[j].value = choose
